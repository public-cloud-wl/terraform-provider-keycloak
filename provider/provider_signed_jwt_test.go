...
var (
	...
	jwtSigningAlg = "RS256"
	jwtClientId   = "terraform-jwt"
	jwtRealm      = "master"
	jwtUrl        = "http://localhost:8080/auth"
)

func TestAccKeycloakProvider_signedJWT_provided(t *testing.T) {
	t.Parallel()

	jwtToken, err := keycloak.NewSignedJWT(
		t.Context(),
		fmt.Sprintf("%s/realms/%s", jwtUrl, jwtRealm),
		jwtClientId,
		jwtSigningAlg,
		jwtSigningKey,
	)
	...
	defer func() {
		...
		os.Unsetenv("KEYCLOAK_JWT_TOKEN")
	}()
...
func TestAccKeycloakProvider_signedJWT_file_provided(t *testing.T) {
	t.Parallel()
	...
	defer func() {
		os.Setenv("KEYCLOAK_CLIENT_ID", "terraform")
		os.Unsetenv("KEYCLOAK_JWT_TOKEN_FILE")
	}()
